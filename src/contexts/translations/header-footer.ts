export const headerFooterTranslations = {
  pt: {
    "navigation": {
      "home": "Home",
      "about": "Sobre",
      "projects": "Projetos",
      "team": "Equipe",
      "contact": "Contato",
      "project_details": "O Projeto",
      "solution": "Solução",
      "technology": "Tecnologia",
      "roadmap": "Roteiro",
      "participate": "Participe",
      "publications": "Publicações",
      "principles": "Princípios",
      "supporters": "Apoiadores",
      "collection": "Coleção",
      "partners": "Parceiros",
      "library": "Biblioteca",
      "pillars": "Pilares",
      "games_galleries": "Jogos & Galerias",
      "content": "Sobre",
      "formats": "Formatos"
    },
    "header": {
      "title": "GIF Labs",
      "subtitle": "Filosofia, Artes, Tecnologia e Educação"
    },
    "footer": {
      "description": "Desenvolvendo a educação crítica para um mundo descentralizado.",
      "copyright": "© 2024 GIFLABS. Todos os direitos reservados.",
      "contact": {
        "title": "Contato",
        "description": "Entre em contato conosco para colaborações, pesquisas, ou mais informações sobre nossos projetos.",
        "group_name": "Grupo Investigação Filosófica",
        "university": "Universidade Federal de Ouro Preto",
        "contact_info": {
          "title": "Informações de Contato",
          "email": "rodrigorlcid@gmail.com",
          "virtualia": "Virtualia Magazine",
          "cnpq": "Grupo registrado no CNPq"
        },
        "research_areas": {
          "title": "Áreas de Pesquisa",
          "philosophy": "Filosofia",
          "web3": "Web3",
          "education": "Educação",
          "blockchain": "Blockchain",
          "ai": "IA",
          "digital_art": "Arte Digital"
        }
      },
      "quote": "Pensar é revolucionário."
    }
  },
  en: {
    "navigation": {
      "home": "Home",
      "about": "About",
      "projects": "Projects",
      "team": "Team",
      "contact": "Contact",
      "project_details": "The Project",
      "solution": "Solution",
      "technology": "Technology",
      "roadmap": "Roadmap",
      "participate": "Participate",
      "publications": "Publications",
      "principles": "Principles",
      "supporters": "Supporters",
      "collection": "Collection",
      "partners": "Partners",
      "library": "Library",
      "pillars": "Pillars",
      "games_galleries": "Games & Galleries",
      "content": "About",
      "formats": "Formats"
    },
    "header": {
      "title": "GIF Labs",
      "subtitle": "Philosophy, Arts, Technology and Education"
    },
    "footer": {
      "description": "Developing critical education for a decentralized world.",
      "copyright": "© 2024 GIFLABS. All rights reserved.",
      "contact": {
        "title": "Contact",
        "description": "Get in touch with us for collaborations, research, or more information about our projects.",
        "group_name": "Grupo Investigação Filosófica",
        "university": "Federal University of Ouro Preto",
        "contact_info": {
          "title": "Contact Information",
          "email": "rodrigorlcid@gmail.com",
          "virtualia": "Virtualia Magazine",
          "cnpq": "GIF (DGP/CNPq)"
        },
        "research_areas": {
          "title": "Research Areas",
          "philosophy": "Philosophy",
          "web3": "Web3",
          "education": "Education",
          "blockchain": "Blockchain",
          "ai": "AI",
          "digital_art": "Digital Art"
        }
      },
      "quote": "Thinking is revolutionary."
    }
  }
};